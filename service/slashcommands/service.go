// Package slashcommands owns command policy and execution against a scoped
// conversation capability. Platform drivers only supply parsed messages.
...
var Registry = []SlashCommand{
	{Name: "auth", Description: "Link your Airlock account", Action: authz.AgentSlashPublic},
	{Name: "cancel", Description: "Stop the current run", Action: authz.AgentSlashPublic},
	{Name: "clear", Description: "Clear conversation context", Action: authz.AgentSlashContext},
	{Name: "compact", Description: "Summarize and compact context", Action: authz.AgentSlashContext},
	{Name: "echo", Description: "Toggle tool output bubbles (on / off / blank=flip)", Action: authz.AgentSlashContext},
}
...
// the central command policy. /clear, /echo, /compact, /cancel route into
...
func TrySlashCommand(
	...
	if name == "start" {
		if err := authz.AuthorizeResolvedAccess(authz.AgentSlashPublic, access); err != nil {
			return SlashCommandResult{Handled: true}, err
		}
	...
	if err := authz.AuthorizeResolvedAccess(entry.Action, access); err != nil {
		return SlashCommandResult{
			Handled: true,
			Reply:   fmt.Sprintf("/%s is not available at your access level.", entry.Name),
		}, nil
	}
