...
func PublishRunTerminal(ctx context.Context, q *dbq.Queries, pubsub *realtime.PubSub, agentID, runID uuid.UUID, status, errMsg string) {
	...
	if err != nil {
		return // Attribution must be known before a terminal payload is published.
	}
	o, err := q.GetRunOrigin(ctx, run.ID)
	if err != nil || o.Actor != "user" || !o.UserID.Valid || o.AgentID != run.AgentID {
		return
	}
	userID, conversationID := uuid.UUID(o.UserID.Bytes).String(), ""
	...
	case "error", "failed", "timeout":
		_ = pubsub.Publish(ctx, agentID, realtime.NewEnvelopeForUser("run.error", topicID, userID, conversationID, &airlockv1.RunErrorEvent{
			RunId: runID.String(),
			Error: errMsg,
		}))
	default:
		_ = pubsub.Publish(ctx, agentID, realtime.NewEnvelopeForUser("run.complete", topicID, userID, conversationID, &airlockv1.RunCompleteEvent{
			RunId: runID.String(),
		}))
	}
}
