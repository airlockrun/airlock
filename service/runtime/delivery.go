...
func PostToConversation(ctx context.Context, deps PostDeps, opts PostOpts) error {
	...
	if opts.TriggerLLM {
		if _, err := execution.ContinuationPrincipal(ctx, q, opts.AgentID, opts.ConversationID, opts.RunID); err != nil {
			return err
		}
	}
	...
	if opts.TriggerLLM && opts.LLMMessage != "" {
		...
		p, err := execution.ContinuationPrincipal(ctx, q, opts.AgentID, opts.ConversationID, opts.RunID)
		if err != nil {
			return err
		}
		access, err := execution.Access(ctx, q, p, opts.AgentID)
		if err != nil {
			return err
		}
		...
		rc, runID, err := deps.Dispatcher.ForwardPrompt(ctx, p, opts.AgentID, input)
		if err != nil {
			return err
		}
		if isBridge {
			...
			PublishRunEvents(ctx, rc, deps.PubSub, deps.DB.Pool(), opts.AgentID, runID, deps.Logger)
		}
