...
type exitToolInput struct {
	Status  string `json:"status" description:"\"success\" (task complete), \"error\" (attempted in-scope work but blocked), or \"refused\" (request is outside agentsdk's scope)"`
...
Set status="success" with a one-paragraph summary when the task is complete.
Set status="error" with a clear explanation of the blocker when you attempted the task but could not complete it.
